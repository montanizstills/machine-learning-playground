...
//             System.out.printf("Word: %s\n", word);
        ...
        writeTensorToBinaryFile(dl4jTensorX, "/src/main/java/resources/tensorX.bin");
        writeTensorToBinaryFile(dl4jTensorY, "/src/main/java/resources/tensorY.bin");
        return new ImmutablePair<>(dl4jTensorX, dl4jTensorY);
    }

    public void writeTensorToBinaryFile(INDArray tensor, String filePath) {
        try {
            Nd4j.saveBinary(
                    tensor,
    ...
    public ImmutablePair<INDArray, INDArray> loadTensorFromBinaryFile(String filePath) {
        try {
            ...
            // INDArray loadedTensorY = Nd4j.readCompressed(new FileInputStream(filePath + "_tensorY.bin.z"));

            return new ImmutablePair<>(loadedTensorX, loadedTensorY);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
    ...
    void train() {
        ...
        System.exit(1);
        Pair<INDArray, INDArray> XY = prepareTrainingData("file/path/names.txt");
